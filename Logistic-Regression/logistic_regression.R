...
setwd("C:/Git_Root/logistic_regression")
...
# collapse all missing values to NA for everwrk
NH11$everwrk <- factor(NH11$everwrk, levels = c("1 Yes", "2 No"))

# check stucture of age_p and r_maritl
str(NH11$age_p)
str(NH11$r_maritl)

# collapse all missing values to NA for r_maritl
...
# Generating predicted values with probabilities
# Create a dataset with predictors set at desired levels
library(dplyr)
minidat <- select(NH11, age_p, r_maritl)
head(minidat, 10)
levels(NH11$r_maritl)
wrkDat <- with(NH11, expand.grid(age_p = c(33, 63),
                                 r_maritl = c("2 Married - spouse not in household",
                                 "4 Widowed", "5 Divorced", "6 Separated", "7 Never married", "8 Living with partner")))

# predict hypertension at those levels
cbind(wrkDat, predict(wrk.out, type = "response", se.fit = T,
                      interval = "confidence", newdata = wrkDat))
